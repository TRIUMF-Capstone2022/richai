...
The capstone project titled "*RICH AI*" floated by TRIUMF is aimed at improving Particle Identification (PID) performance of the Ring Imaging Cherenkov (RICH) by developing a classification model based on advanced Machine Learning (ML) and Deep Learning (DL) methods.

The background of the project lies in the NA62 experiment at CERN (European organization for nuclear reasearch), which aims to measure an ultra-rare meson decay reaction to identify pion, which is produced along with muon and positron. The main challenge is to increase pion efficiency, that is, the fraction of the matched detected pions out of total number of pions or the true positive rate.

The expected data product of this project is a Python package that applies a combination of ML and DL techniques to accurately classify the particle produced in a decay event. The initial data provided by TRIUMF for this project is from the NA62 experiments run in 2018 and contains particle tagged examples of approximately 11.5 million muon, pion and positron type events. Each event data consists of features specific to the decay such as the particle momentum, CHOD time and the PMT data comprising hit times, and X and Y positions of the photons produced as the subatomic particle passes through the RICH detector. More data, if needed, is to be provided by TRIUMF.

The discussed modeling approach is comprised of developing models from simpler to complex ones, starting with a gradient-boosted tree (LightGBM or XGBoost) as our baseline model. Going further, due to the sparse and scattered nature of the images from the experiment, the neural network structures that are proposed to be explored are PointNet and Graph Convolution Neural networks (CNN). These algorithms are specifically designed to handle the specific kind of image data as described above, and are preferred over traditional CNN. For assessment of the models' performances, pion efficiency and Receiver Operating Characteristic (ROC) curve are proposed as evaluation metrics .

The project is proposed to be executed in 6 phases: proposal, implementation, evaluation, code refactoring, documenting and reporting, and the final handover.
...
6.  The CHOD detector shown in Figure X [@anzivino2020light] records the time that the particle decay occurs.
